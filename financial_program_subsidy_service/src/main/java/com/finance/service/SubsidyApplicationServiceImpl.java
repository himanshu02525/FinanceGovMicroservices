...
//        String email = "hmcreationhimanshu011@gmail.com";
		...
		NotificationRequestDto notification = NotificationRequestDto.builder().userId(id).entityId(saved.getEntityId())
				...
				.message("Your subsidy application has been submitted successfully.").build();
	...
	private SubsidyApplicationResponse toResponse(SubsidyApplication app) {
		return new SubsidyApplicationResponse(app.getApplicationId(), app.getEntityId(), app.getSubmittedDate(),
				app.getProgram().getProgramId(), app.getStatus());
	}
	...
	@Override
	public List<SubsidyApplicationResponse> fetchByProgram(Long programId) {

		List<SubsidyApplication> programs = applicationRepository.findByProgramId(programId);
